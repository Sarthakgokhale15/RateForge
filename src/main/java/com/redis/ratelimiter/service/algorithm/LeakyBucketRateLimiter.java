...
@Component
public class LeakyBucketRateLimiter extends AbstractRedisRateLimiter implements RateLimiterAlgorithm {

    private final RedisScript<String> leakyBucketScript;
    ...
    public RateLimitResult allow(String identityKey, RateLimitPolicy policy) {
        ...
        double leakRate = Math.max(0.0001d, policy.getLeakRatePerSecond());
        long requested = Math.max(1, policy.getRequestedTokens());
        long now = nowMillis();
        long ttl = Math.max(2, policy.getWindowSeconds() * 2);

        String redisKey = Objects.requireNonNull(key("leaky_bucket", identityKey));
        String result = redisTemplate.execute(
            Objects.requireNonNull(leakyBucketScript),
